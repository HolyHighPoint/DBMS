...
def table_structure(table):
    ds_table = dataset[table]  #在前端的作用存疑
    model_class = ds_table.model_class  #存疑
...
def add_column(table):
        ...
        if name and col_type in column_mapping:
            migrate(
                migrator.add_column(
                    table,
                    name,
                    column_mapping[col_type](null=True)))   #修改此处，增加表属性的过程;此外，migrate的参数为operation类
    ...
    return render_template(            #对GET方法的响应，不修改
        'add_column.html',
        col_type=col_type,
...
def drop_column(table):
    request_data = get_request_data()
    name = request_data.get('name', '')
    columns = dataset.get_columns(table)                 #从此处得到属性的元组
    column_names = [column.name for column in columns]   #列名称的元组

    if request.method == 'POST':
        if name in column_names:
            migrate(migrator.drop_column(table, name))  #修改此处，删去表属性的过程
...
def rename_column(table):
    ...
    if request.method == 'POST':
        if (rename in column_names) and (rename_to not in column_names):
            migrate(migrator.rename_column(table, rename, rename_to))   #需要修改：在数据库中修改一列的名字
...
def drop_trigger(table):
    request_data = get_request_data()
    name = request_data.get('name', '')
    triggers = dataset.get_triggers(table)      #修改:从数据库中得到触发器的列表
    trigger_names = [trigger.name for trigger in triggers]

    if request.method == 'POST':
        if name in trigger_names:
            dataset.query('DROP TRIGGER "%s";' % name)      #修改：删除一个触发器
